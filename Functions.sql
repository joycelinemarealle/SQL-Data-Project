...
|| '-hello--'
|| substring (address, position ( ' ' in address) +1 , length(address ) - position ( ' ' in address) +1 )
from authors;
...
left (address, position (' ' in address) -1) 
|| '-hello--'
|| substring ( address, position (' ' in address)+1)
from authors;


select address,
regex_replace(address, ' ', '-hello--') as "Address" 
from authors;
...
select coalesce (price, random() * 100 +1) :: numeric :: money)
from titles;

--Print all phone numbers from the authors table without the three digit area code.
select substr ( phone, 5)
from authors;

select right ( phone, length(phone)-4)
from authors;

--Capitalise the second character in all last names from the authors table.
select au_lname, left ( au_lname, 1) || upper (substr(au_lname, 2, 1)) || right(au_lname,length(au_lname)-2)
from authors;

select au_lname, left ( au_lname, 1) || upper (substr(au_lname, 2, 1)) || substri(au_lname,3)
from authors;